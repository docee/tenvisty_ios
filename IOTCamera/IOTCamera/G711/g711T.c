 ...
 *
 */
unsigned char linear2alawT(int pcm_val)	/* 2's complement (16-bit range) */
{
	int		mask;
	int		seg;
	unsigned char	aval;
 ...
 *
 */
unsigned char linear2ulawT(int pcm_val)	/* 2's complement (16-bit range) */
{
	int		mask;
	int		seg;
	unsigned char	uval;
...
int linear2alawTT(int	pcm_val)        /* 2's complement (16-bit range) */
...
int g711a_encode(unsigned char g711_data[], const short amp[], int len)
{
    ...
    for (i = 0;  i < len;  i++)
	{
        g711_data[i] = linear2alawTT(amp[i]);
    }
...
int g711u_encode(unsigned char g711_data[], const short amp[], int len)
{
    ...
    for (i = 0;  i < len;  i++)
	{
        g711_data[i] = linear2ulawT(amp[i]);
    }
...
short decodeT(unsigned char alaw)
{
    alaw ^= 0xD5;
    int sign = alaw & 0x80;
