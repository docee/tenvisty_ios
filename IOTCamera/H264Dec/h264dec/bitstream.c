                       ...
                       uint32_t code_prefix, int n_prefix, int flags)
{
...
#ifdef DEBUG_VLC
    av_log222(NULL,AV_LOG_DEBUG,"new table index=%d size=%d code_prefix=%x n=%d\n",
    ...
    for(i=0;i<nb_codes;i++) {
        ...
        av_log222(NULL,AV_LOG_DEBUG,"i=%d n=%d code=0x%x\n", i, n, code);
#endif
            ...
            if (n <= table_nb_bits) {
                ...
                for(k=0;k<nb;k++) {
                    if(flags & INIT_VLC_LE)
                        j = (code >> n_prefix) + (k<<n);
#ifdef DEBUG_VLC
                    av_log222(NULL, AV_LOG_DEBUG, "%4x: code=%d n=%d\n",
                           j, i, n);
#endif
                    if (table[j][1] /*bits*/ != 0) {
                        av_log222(NULL, AV_LOG_ERROR, "incorrect codes\n");
                        return -1;
                    }
                    table[j][1] = n; //bits
                    table[j][0] = symbol;
...
#ifdef DEBUG_VLC
                av_log222(NULL,AV_LOG_DEBUG,"%4x: n=%d (subtable)\n",
                       j, n);
#endif
                /* compute table size */
                n1 = -table[j][1]; //bits
             ...
             int flags)
{
...
#ifdef DEBUG_VLC
    av_log222(NULL,AV_LOG_DEBUG,"build table nb_codes=%d\n", nb_codes);
#endif
    ...
    if((flags & INIT_VLC_USE_NEW_STATIC) && vlc->table_size != vlc->table_allocated)
        av_log222(NULL, AV_LOG_ERROR, "needed %d had %d\n", vlc->table_size, vlc->table_allocated);
    return 0;
}
