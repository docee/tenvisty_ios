...
#import "SDCard_AoniViewController.h"

@interface SDCard_AoniViewController (){
    NSInteger freeSize;
    NSInteger totalSize;
    BOOL isFormatting;
}
...
@implementation SDCard_AoniViewController
...
-(void)getFormatStatus{
    [self.camera sendIOCtrlToChannel:0 Type:IOTYPE_USER_IPCAM_GET_FORAMT_RESULT_REQ Data:(char *)nil DataSize:0];
}
...
- (void)camera:(NSCamera *)camera _didReceiveIOCtrlWithType:(NSInteger)type Data:(const char*)data DataSize:(NSInteger)size{
    switch (type) {
        ...
        case IOTYPE_USER_IPCAM_FORMATEXTSTORAGE_RESP:{
            ...
            break;
        case IOTYPE_USER_IPCAM_GET_FORAMT_RESULT_RESP:{
            SMsgAVIoctrlGetFormatStatusResp *resp = (SMsgAVIoctrlGetFormatStatusResp*)data;
            if(resp->result == 1){
