...
#import "SystemSettingTable_HichipViewController.h"
...
@interface SystemSettingTable_HichipViewController (){
...
@implementation SystemSettingTable_HichipViewController
...
-(void)doReset{
    ...
    [self.camera sendIOCtrlToChannel:0 Type:HI_P2P_SET_RESET Data:(char*)nil DataSize:0];
}
-(void)doReboot{
    rebootState = 0;
...
-(void) requestOnlineVersion {
    ...
    // App Name
    NSString *urlStr = [NSString stringWithFormat:@"http://%@/%@/goke_update.html",host,[informationDic objectForKey:@"CFBundleDisplayName"]];
...
- (void) recvUrlData:(NSData*)data {
    ...
    for (NSDictionary *dic in jsonLists) {
        ...
        isUpdate = [self isUpdateNewVersion:ver oldVersion:self.originCamera.deviceInfoExt.aszSystemSoftVersion];
...
-(void)doStartUpgrade{
    if(self.download){
        HI_P2P_S_SET_DOWNLOAD *req=  [self.download model];
        [self.camera sendIOCtrlToChannel:0 Type:HI_P2P_SET_DOWNLOAD Data:(char*)req DataSize:sizeof(HI_P2P_S_SET_DOWNLOAD)];
        free(req);
        req = nil;
    }
}
