...
-(void)setup{
    ...
    for (LocalPictureInfo *model in self.images) {
        ...
        [imageNames addObject:model.path];
...
- (UILabel *)indexTitle {
    if (!_indexTitle) {
        ...
        _indexTitle.text = LOCALSTR(@"Local Picture");
...
- (void)deletePicture {
    ...
    for (LocalPictureInfo *model in self.images) {
        ...
        if ([model.path isEqualToString:self.loop.deleteName]) {
            ...
            [GBase deletePicture:self.camera name:model.path];
...
- (void)presentAlertControllerAfterIOS8 {
    ...
    UIAlertAction *actionDelete = [UIAlertAction actionWithTitle:LOCALSTR(@"Delete") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self deletePicture];
    }];
