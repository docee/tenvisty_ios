...
@implementation TwsProgress

+ (instancetype)sharedProgress {
    
    static TwsProgress *progress = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        
        progress = [[TwsProgress alloc] init];
    });
...
+ (void)showProgressSize:(CGSize)size Color:(UIColor *)color dissmissTime:(NSTimeInterval)seconds {
    ...
    TwsProgress *p = [TwsProgress sharedProgress];
...
+ (void)showText:(NSString *)text Size:(CGSize)size Color:(UIColor *)color dissmissTime:(NSTimeInterval)seconds {
    ...
    CGRect r = [p rectOfText:text WithFont:[UIFont systemFontOfSize:18]];
    CGFloat w = size.width > r.size.width+10 ? size.width : r.size.width+10;
    CGFloat h = size.height > r.size.height+5 ? size.height : r.size.height+5;
...
+ (void)showText:(NSString *)text durationTime:(NSInteger)time{
    [self showText:text withSize:CGSizeMake(100, 50) durationTime:time];
}
+ (void)dismiss {
...
//        NSLog(@"class:%@", [NSString stringWithUTF8String:object_getClassName(view)]);
        ...
        if ([view isKindOfClass:[TwsProgress class]]) {
